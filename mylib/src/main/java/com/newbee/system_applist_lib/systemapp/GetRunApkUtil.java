...
public class GetRunApkUtil {
    ...
    public String getTopPagePackName(Context context){
        String topPagePackName="";
        try {
            ...
            if (tasksInfo.size() > 0) {
                // 应用程序位于堆栈的顶层
              topPagePackName=   tasksInfo.get(0).topActivity
                        .getPackageName();

            }
        }catch (Exception e){
        }
        return topPagePackName;
    }


    public boolean isTopPage(Context context,String packageName) {
       try {
           if(TextUtils.isEmpty(packageName)){
               return false;
           }
           return packageName.equals(getTopPagePackName(context));
       }catch (Exception e){
           return false;
       }
    }

    public boolean isTopActivity(Context context,String packageName,String className) {
        try {
            ActivityManager activityManager = (ActivityManager) context.getApplicationContext()
                    .getSystemService(Context.ACTIVITY_SERVICE);
            List<ActivityManager.RunningTaskInfo> tasksInfo = activityManager.getRunningTasks(1);
